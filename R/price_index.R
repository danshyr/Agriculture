...
df_10dayPeriod = read.xlsx ("price.xlsx", sheetIndex = 1, header = F, startRow =2, endRow = 441, stringsAsFactors = FALSE)
# Set the missing values as -1's
df_10dayPeriod$X2[df_10dayPeriod$X2=="-"] = -1
df_10dayPeriod$X3[df_10dayPeriod$X3=="-"] = -1
df_10dayPeriod$X4[df_10dayPeriod$X4=="-"] = -1
df_10dayPeriod$X4[df_10dayPeriod$X5=="-"] = -1

library(lubridate)
# Create a new, empty data frame for expanded daily records in the future
df = data.frame(X1 = as.Date(character()), X2 = numeric(), X3 = numeric(),X4 = numeric(),X5 = numeric())
...
for (i in 1:nrow(df_10dayPeriod)){
  ...
  for (j in 1:30){
    ...
    #print (currentDate)
    newRow = data.frame(X1 = currentDate, X2 = df_10dayPeriod$X2[i], X3 = df_10dayPeriod$X3[i], X4 = df_10dayPeriod$X4[i], X5 = df_10dayPeriod$X5[i], stringsAsFactors = FALSE)
    df = rbind(df, newRow)
...
write.xlsx(df, "price-change2.xlsx")
