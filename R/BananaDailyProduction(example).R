...
priceMerge = na.omit(merge(banana_production_price,market_banana,by.x='date',all.x=TRUE))
priceMergeDate = priceMerge[priceMerge$date>='2004-01-01' & priceMerge$date<='2014-12-31',]
v = c(1,9)
tradeRaw = priceMergeDate[,v]

#take production of year
c = c(1,9)
productionB = production[production$fruits=='é¦™è??',c]
#normalize the trade
...
#write out
write.xlsx(NewNorlize,file='BananaProductionDaily.xlsx')
