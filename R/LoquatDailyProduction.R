...
priceMerge = na.omit(merge(loquat_production_price,market_loquat,by.x='date',all.x=TRUE))
priceMergeDate = priceMerge[priceMerge$date>='2004-01-01' & priceMerge$date<='2014-12-31',]
v = c(1,9)
tradeRaw = priceMergeDate[,v]

productionB = production[production$fruits=='枇杷',v]
...
#write out
write.xlsx(NewNorlize,file='LoquatProductionDaily.xlsx')
