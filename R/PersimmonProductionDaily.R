...
priceMerge = na.omit(merge(persimmon_production_price,market_persimmon,by.x='date',all.x=TRUE))
priceMergeDate = priceMerge[priceMerge$date>='2004-01-01' & priceMerge$date<='2014-12-31',]
v = c(1,9)
tradeRaw = priceMergeDate[,v]

productionB = production[production$fruits=='柿',v]
...
#write out
write.xlsx(NewNorlize,file='PersimmonProductionDaily.xlsx')
